...
public class PendingMetricsBinder implements MeterBinder {
    ...
    public void bindTo(MeterRegistry registry) {
        ...
        log.info("PendingMetricsBinder: 已为 {} 个 Pending 实例注册指标",
    ...
    private void registerPendingGauges(MeterRegistry registry, AbstractLazyPending pending,
        ...
        // 信号量剩余许可数（=可以继续处理的批次槽位数量）
        Gauge.builder("austin.pending.semaphore.permits", pending,
                        p -> (double) p.getInFlightBatches().availablePermits())
                .description("Pending 信号量剩余可用许可数（许可耗尽表示下游满载）")
                .tag(TAG_PENDING_CLASS, className)
                .register(registry);
