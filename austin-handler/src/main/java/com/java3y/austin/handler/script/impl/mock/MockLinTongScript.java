...
@Profile("test")
@Slf4j
public class MockLinTongScript extends AbstractMockScript {
        ...
        try {
            ...
            switch (status) {
                case MockStatusConstant.SUCCESS:
                    log.info("模拟调用LinTong发送短信成功");
                    break;
                case MockStatusConstant.TIMEOUT:
                    throw new NetWorkTimeoutException("模拟LinTong发送短信超时");
                default:
                    throw new RuntimeException("模拟LinTong发送短信失败");
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new RuntimeException("模拟LinTong发送短信过程中被中断", e);
        }
